...
def show_lesson_screen(stdscr: curses.window):
	lesson_file_path = get_address("lesson")
	f = open(lesson_file_path)
	lessons = json.load(f)
	SELECTED_LESSON = 1
		...
		try:
			...
			if key == curses.KEY_UP:
				if SELECTED_LESSON - 1 < 10:
					SELECTED_LESSON = min(
						10 * (max_lesson // 10) + SELECTED_LESSON, max_lesson
					)
				else:
					SELECTED_LESSON -= 10
				...
				if SELECTED_LESSON > max_lesson:
					if (
						min(SELECTED_LESSON, max_lesson) == max_lesson
						and SELECTED_LESSON - 10 != max_lesson
					):
			...
			stdscr.clear()
			stdscr.addstr(0, 0, "Increase the size of screen")
			stdscr.refresh()